...
const getBaseViteConfig = async (ladleConfig, configFolder, viteConfig) => {
    ...
    Object.keys(ladleConfig).forEach((configKey) => {
        if (_removedLadleConfigOptions.includes(configKey)) {
            console.error(`ERROR: ${configKey} was removed from the Ladle config in v1. Move it to vite.config.js. https://ladle.dev/docs/config`);
            oldKeyUsed = true;
        }
    });
    oldKeyUsed && process.exit(1);
    const { userViteConfig, hasReactPlugin, hasReactSwcPlugin, hasTSConfigPathPlugin, } = await getUserViteConfig(viteConfig.build ? "build" : "serve", viteConfig.mode || "production", ladleConfig.viteConfig);
    ...
    const inladleMonorepo = fs.existsSync(path.join(__dirname, "../../../../e2e/addons/package.json"));
    ...
    const storyEntries = (await globby(Array.isArray(ladleConfig.stories)
        ? ladleConfig.stories
        : [ladleConfig.stories])).map((story) => path.join(process.cwd(), story));
    /**
    ...
    const config = {
        ...
        configFile: false,
        publicDir: typeof userViteConfig.publicDir === "undefined"
        ...
        root: getAppRoot(),
        css: {
            postcss: userViteConfig.css && userViteConfig.css.postcss
