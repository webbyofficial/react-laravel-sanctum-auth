...
const getStoryList = (entryData) => {
    ...
    const output = generate(t.exportNamedDeclaration(t.variableDeclaration("let", [
        t.variableDeclarator(t.identifier("stories"), t.objectExpression(storyIds.map((story) => {
            let paramsAst = null;
            if (storyParams[story]) {
                paramsAst = t.objectProperty(t.identifier("meta"), template.ast(`const foo = ${JSON.stringify(storyParams[story])}`).declarations[0].init);
            }
            return t.objectProperty(t.stringLiteral(story), t.objectExpression([
                t.objectProperty(t.identifier("component"), t.identifier(story.replace(new RegExp(storyDelimiter, "g"), storyEncodeDelimiter))),
