                    ...
                    if (t.isImportSpecifier(specifier)) {
                        ...
                        if (specifierName === "Meta") {
                            refPaths.forEach((refPath) => {
                                ...
                                metaOccurances++;
                                if (metaOccurances > 1) {
                                    throw new Error("Only one Meta component can be defined per file");
                                }
                                const metaAttributes = refPath.parentPath.container.openingElement.attributes;
                                const exportDefaultProps = [];
                                let storyName = "Readme";
                                metaAttributes.forEach((attr) => {
                                    if (attr.value.type === "JSXExpressionContainer") {
                                        exportDefaultProps.push(t.objectProperty(t.identifier(attr.name.name), attr.value.expression));
                                    }
                                    if (attr.value.type === "StringLiteral" &&
                                        ...
                                        if (titleParts.length > 1) {
                                            ...
                                            exportDefaultProps.push(t.objectProperty(t.identifier(attr.name.name), t.stringLiteral(titleParts.join("/"))));
                                        }
                                        else {
                                ...
                                if (exportDefaultProps.length > 0) {
                                    program.pushContainer("body", t.exportDefaultDeclaration(t.objectExpression(exportDefaultProps)));
                                }
                                program
                                    .get("body")[0]
                                    .container.push(t.expressionStatement(t.assignmentExpression("=", t.memberExpression(t.identifier("MDXContent"), t.identifier("storyName")), t.stringLiteral(storyName))));
                            });
                        }
                        if (specifierName === "Story") {
                            refPaths.forEach((refPath, index) => {
                                ...
                                program.pushContainer("body", t.exportNamedDeclaration(t.variableDeclaration("const", [
                                    t.variableDeclarator(t.identifier(identifier), t.arrowFunctionExpression([], t.jsxFragment(t.jsxOpeningFragment(), t.jsxClosingFragment(), content))),
                                ])));
                                refPath.container.attributes.forEach((attr) => {
                                    program.pushContainer("body", t.expressionStatement(t.assignmentExpression("=", t.memberExpression(t.identifier(identifier), t.identifier(attr.name.name === "name"
                                        ? "storyName"
                                        : attr.name.name)), attr.value.type === "JSXExpressionContainer"
...
const prepare = async (code, filename, transformMdx = false) => {
    ...
    let result = output?.code?.replace("export default function MDXContent(", "export function MDXContent(");
