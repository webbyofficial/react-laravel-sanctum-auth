...
function mdxPlugin(opts) {
    ...
    return {
        ...
        configResolved: ({ plugins }) => {
            reactPluginTransform = plugins.find((p) => p.name === "vite:react-babel" && typeof p.transform === "function")?.transform;
        ...
        async transform(value, path) {
            ...
            if (path.endsWith(".mdx")) {
                ...
                if (!isDev) {
                    return (await transformWithEsbuild(code, filepath.replace(".mdx", ".jsx"), {
                        jsx: "automatic",
                    })).code;
                }
                // trick esbuild and vitejs/plugin-react into compiling it as jsx
                const filename = `${filepath}${querystring ? "&ext=.jsx" : "?ext=.jsx"}`;
                if (!usesReactPluginSwc && !reactPluginTransform) {
                    throw new Error(`You need to install and use @vitejs/plugin-react-swc or @vitejs/plugin-react so ${filename} can be compiled.`);
                }
                if (reactPluginTransform) {
                    return await reactPluginTransform((await transformWithEsbuild(code, filename)).code, filepath.replace(".mdx", ".jsx"));
                }
