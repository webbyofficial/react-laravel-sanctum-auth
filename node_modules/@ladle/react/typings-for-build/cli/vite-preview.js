...
const vitePreview = async (config, configFolder) => {
    try {
        ...
        let openBrowserValue = viteConfig.preview && viteConfig.preview.open
        ...
        const serverUrl = `${previewServer.config.preview.https ? "https" : "http"}://${previewServer.config.preview.host || "localhost"}:${config.previewPort}${config.base ? config.base : ""}`;
        console.log(boxen(`🥄 Ladle.dev previewed at ${serverUrl}`, {
            padding: 1,
            margin: 1,
            borderStyle: "round",
            borderColor: "blue",
