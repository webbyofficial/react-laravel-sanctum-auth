...
const build = async (params = {}) => {
    ...
    await fs.writeFile(path.join(process.cwd(), config.outDir, "meta.json"), jsonContent);
    console.log("✓  Meta.json successfully created.");
    const folderSize = await getFolderSize(path.join(process.cwd(), config.outDir));
    const stopTime = performance.now();
    const inSeconds = (stopTime - startTime) / 1000;
    console.log(`⏱️  Ladle finished the production build in ${Number(inSeconds).toFixed(0)}s producing ${folderSize} MiB of assets.`);
