...
const ArgsProvider = ({ component, args, argTypes, }) => {
    const { globalState, dispatch } = useLadleContext();
    const actionLogger = (name) => (event) => {
        dispatch({
    ...
    React.useEffect(() => {
        ...
        argTypes &&
            Object.keys(argTypes).forEach((argKey) => {
                ...
                if (ALLOWED_ARGTYPES.indexOf(argValue.control.type) === -1) {
                    throw new Error(`only ${ALLOWED_ARGTYPES.join(", ")} argTypes are supported now. For strings, booleans and numbers use just args.`);
                }
                if (argValue.control.type === "background") {
                    bgControls++;
                    if (bgControls > 1) {
                        throw new Error("There can be only single argType with the type background since it's used to change Ladle's background color.");
                    }
                }
                controls[argKey] = {
                    name: argValue.name,
        ...
        if (Object.keys(controls).length) {
            ...
            if (Object.keys(controls).some((key) => !globalState.control[key] ||
